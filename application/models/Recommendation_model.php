...
class recommendation_model extends CI_Model
{
  ...
  function get_data_sidebar()
  {
    $this->db->limit(5);
    $this->db->from($this->table);
    $this->db->join('produk', 'recommendation.produk = produk.id_produk');
    return $this->db->get()->result();
  }

  function get_combo_recommendation()
  {
    $this->db->order_by('judul_recommendation', 'ASC');
    ...
    if($query->num_rows() > 0){
      ...
      {
        $data[$row['id_recommendation']] = $row['judul_recommendation'];
      }
      return $data;
    }
  }
  ...
  function get_all_recommendation_sidebar()
  {
    $this->db->limit(5);
    $this->db->where('publish','ya');
    $this->db->order_by($this->id, $this->order);
  ...
  function get_all_komentar_sidebar()
  {
    ...
    $this->db->join('komentar', 'recommendation.id_recommendation = komentar.id_recommendation');
    return $this->db->get()->result();
  }
  ...
  // get all
  function get_cari_recommendation()
  {
    $cari_recommendation = $this->input->post('cari_recommendation');

    $this->db->like('judul_recommendation', $cari_recommendation);
    return $this->db->get($this->table)->result();
  }
