...
// cm.push(cosineSim(barangada, tetangga, 9))
// usm.push(userSim(barangada, tetangga, 9))
...
var p =0;
var bwh = 0;
// var cm1 = ConvertToDim1(cm);
// var usm1 = [];
// _.each(cm, function (c) {
//   _.each(c, function (cc) {
//     // usm1.push(cc)
//     if (cc.userSim != NaN || cc.userSim != undefined){
//       usm1.push(cc);
//     }
//   })
// })
//Sorting CosineSim
// var cmm = _.chain(cm1).sortBy(function (item) { return item.untuk_item; })
//                         .sortBy(function (item) {
//                           return item.tetangga;
//                         })
// var usmm = _.chain(usm1).sortBy(function (item) {
//                           return item.untuk_item; })
//                         .sortBy(function (item) {
//                             return item.tetangga;
//                         })

// console.log("ini usm");
// console.log(usmm._wrapped);
// console.log("ini cmm");
// console.log(cmm._wrapped);
// _.each(cmm, function (c) {
//   console.log(c);
// })

// _.each(cmm._wrapped,function (c) {
//   _.each(usmm._wrapped, function (s) {
//     if (c.tetangga == s.tetangga) {
//       if (c.untuk_item == s.untuk_item) {
//         ats += ((s.userSim*c.cosineSim)*(s.rt-s.arub))
//         bwh += Math.abs(s.userSim*c.cosineSim)
//         p = s.arua + (ats/bwh)
//         if(c.length == 1 ){
//           prediksi.push({user: user, item: s.untuk_item, ratingp: p})
//         }
//       }
//       else
//       {
...
//Handle jika cosineSim.length == 0   || Nanti ambil dari rating tertinggi yang ada di user
// if (cm.length < 1) {
//   _.each(barangada, function (b) {
//
//   })
...
app.get('/', function(req, res){
  ...
  var hasil_algo = []
    ...
    axios.get(url).then(function (response) {
      ...
      res.send(hasil)
    //   getData().then(function (resa) {
    //     // res.send(resa)
    //     var data_rating = resa // ini dari semua rating
    //     var hasil_algo = hasil // ini hasil algonya
    //     var davkon = []
    //     for (var i = 0; i < hasil_algo.length; i++) {   // yang di length itu hapus yang -1 nya
    //       for (var j = 0; j < data_rating.length; j++) { // yang di length itu hapus yang -1 nya
    //         if(hasil_algo[i].item == data_rating[j].item_id)
    //         {
    //           data_rating[j].rating = hasil_algo[i].ratingp
    //           davkon.push(data_rating[j])
    //         }
    //       }
    //     }
    //     var sorting = _.sortBy(data_rating,function (a)
    //     {
