...
<html lang="en" dir="ltr">
  <head>
    <script src='http://localhost/uploads/jquery.min.js'></script>
    <script src='http://localhost/uploads/underscore-min.js'></script>
    <!-- <script src="http://localhost/udemy/algo_sample.js"></script> -->
    <!-- <script src="http://localhost/udemy/algo.js"></script> -->
    <meta charset="utf-8">
    <title></title>
  </head>
</html>
...
function initTest(){
  algoritma(); //Mencari
  bebquganteng(); //Mencari userSim + cosineSim
  CariPandoraBox();
  bebekjadiAngsa(); //Mencari Predikat
  ratingAllItem = _.sortBy(ratingAllItem,"rating").reverse();
	...
	_.each(theReal,function (t) {
		...
		pengujian.push({id_produk:t.id_produk,rating: t.rating, pengujian: a})
	})
  ...
  document.write(JSON.stringify(ratingAllItem));
}
theReal = _.sortBy(theReal,'rating');
...
var MissingItems = [];
function CariPandoraBox(){
 // console.log(kitaMiripLoh.length);
    for(var j=0; j<kitaMiripLoh.length; j++){
      for (var k=0; k<kitaMiripLoh[j].allProduct.length; k++){
        for(var i=0; i<barangada.length; i++){
          //console.log("KitaMiripLoh All Product : " + kitaMiripLoh[j].allProduct[k].item + " Barang Ada item : " + barangada[i].item);
          if(barangada[i].item != kitaMiripLoh[j].allProduct[k].item && kitaMiripLoh[j].allProduct[k].nrating>=3) {
					// if(barangada[i].item != kitaMiripLoh[j].allProduct[k].item) {
            MissingItems.push(kitaMiripLoh[j].allProduct[k]);
            break;
          }
        }
      }
...
var ratingTarget = [];
var daftar_item;
var daftar_user = _.unique(_(dataAll).pluck('id_users'));
...
var features = ['id_produk','id_users','kat_id','umur'];
    ...
    function algoritma(){
      $.ajax({
        type: 'ajax',
        // url: 'http://localhost/algoritma/halo.php',
        // url: 'http://localhost/udemy/get_data_algoritma.php',
				// url: 'http://localhost/algoritma/get_data.php',
        // url: 'http://localhost/udemy/davtod.php',
				url: 'http://localhost/uploads/test.php',
        async: false,
        dataType: 'json',
        success: function(data){
          ...
          // console.log(test);
					...
					dataAll = data;
					// var user = 167; //var user
					// var user = 748397;
					var user = 10; //var user
					...
					rules = _.sortBy(rules,'id_users');
         //console.log(data);
//          console.log(id3(data_,'rating',features));
					//console.log("BISAAA==================================================================");
          // var user = 748397; //var user
					test = id3(_(rules),'rating',features);
          hasil.push(cariRekomendasi(rules,user,test))
					bisa = aturan(test,user);
					// console.log(bisa);
        }
      });
    }
